...
public partial class Driver : Window
{
    ...
    private void LoadLeaveRequests()// метод для загрузки данных
    {
        using (MySqlConnection conn = new MySqlConnection(_connString))// создаем соединение
        {
            ...
            string query = "SELECT * FROM driver";// формируем SQL-запрос
            ...
            {
                List<Driver1> leaveRequests = new List<Driver1>();// создаем список для хранения данных

                while (reader.Read())// читаем данные
                {
                    Driver1 leaveRequest = new Driver1();// создаем объект
                    leaveRequest.DriverID = reader.GetInt32("DriverID");
                    leaveRequest.Name = reader.GetString("Name");
                    leaveRequest.Rate = reader.GetString("Rate");
